...
func (sim *FluidSim) CalculateAttractionForces(dt float64) {
	...
	// Collect cell indices to parallelize iteration
	cellKeys := make([]spatial.CellIndex, 0, len(sim.Grid.CellMap))
	for k := range sim.Grid.CellMap {
		cellKeys = append(cellKeys, k)
	}

	parallelFor(0, len(cellKeys), func(idx int) {
		cellIdx := cellKeys[idx]
		indices, ok := sim.Grid.CellMap[cellIdx]
		if !ok { // Should always be found, but good to check
			return
		}
			...
			for ny := cellY - 1; ny <= cellY+1; ny++ {
				...
				for _, i := range indices {
					...
					for _, j := range neighborIndices {
						...
						atomicAddFloat64(&p1.Vx, accX1)
						atomicAddFloat64(&p1.Vy, accY1)
						atomicAddFloat64(&p2.Vx, accX2)
						atomicAddFloat64(&p2.Vy, accY2)
					}
				}
			}
		}
